...
class MMD_loss(nn.Module):
    def __init__(self, kernel_mul = 2.0, kernel_num = 5):
        ...
        self.fix_sigma = None
        self.samplenum = 500  # 减小采样数量
        self.batch_size = 50  # 添加批处理大小
        return

    def compute_bandwidth(self, source, target):
        """计算带宽的优化版本"""
        n_samples = min(100, int(source.size()[0]))  # 使用更小的样本集计算带宽
        source_sample = source[:n_samples]
        target_sample = target[:n_samples]
        
        # 使用矩阵运算计算距离
        source_dist = torch.cdist(source_sample, source_sample, p=2)
        target_dist = torch.cdist(target_sample, target_sample, p=2)
        cross_dist = torch.cdist(source_sample, target_sample, p=2)
        
        # 计算上三角矩阵的距离
        distances = torch.cat([
            source_dist.triu(diagonal=1).flatten(),
            target_dist.triu(diagonal=1).flatten(),
            cross_dist.flatten()
        ])
        
        bandwidth = torch.mean(distances)
        return bandwidth
    ...
    def guassian_kernel(self, source, target, kernel_mul=2.0, kernel_num=5, fix_sigma=None):
        if fix_sigma:
            ...
            bandwidth = self.compute_bandwidth(source, target)
        ...
        kernel_matrix = torch.zeros(n_source + n_target, n_source + n_target, device=source.device)
        
        # 使用矩阵运算计算距离矩阵
        source_dist = torch.cdist(source, source, p=2)
        target_dist = torch.cdist(target, target, p=2)
        cross_dist = torch.cdist(source, target, p=2)
        
        # 填充核矩阵
        kernel_matrix[:n_source, :n_source] = source_dist
        kernel_matrix[n_source:, n_source:] = target_dist
        kernel_matrix[:n_source, n_source:] = cross_dist
        kernel_matrix[n_source:, :n_source] = cross_dist.t()
        
        # 计算高斯核
        kernel_val = torch.zeros_like(kernel_matrix)
        for bw in bandwidth_list:
            kernel_val += torch.exp(-kernel_matrix / bw)
    ...
    def forward(self, source, target):
        ...
        # 随机采样
        if source.size(0) > self.samplenum:
            idx = torch.randperm(source.size(0))[:self.samplenum]
            source = source[idx]
        if target.size(0) > self.samplenum:
            idx = torch.randperm(target.size(0))[:self.samplenum]
            target = target[idx]
...
#         idx = tmp_list.pop(0)
#         if struct[idx]['split']:
#             tmp_list.append(idx+1)
#             tmp_list.append(idx+2)
#         elif struct[idx]['extended']:
#             tmp_list.append(idx+1)
#         else:
#             leaf_list.append(idx)
#     return leaf_list
...
def get_params_node(grow, node_idx, model, class_aware, grow_class=False):
