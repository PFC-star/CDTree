...
from utils.ops import get_leaf_nodes, get_past_leaf_nodes, get_path_to_root
from utils.utils import define_solver
...
class Tree(nn.Module):
    ...
    def __init__(self,
        ...
        for i, node in enumerate(tree_modules):
            ...
            if self.class_aware:
                ## todo 需要手动改的
                if self.args.solver_ver == 2:
                    weights_1 = copy.deepcopy(node["classifier"].fc1.weight.data)
                    bias_1 = copy.deepcopy(node["classifier"].fc1.bias.data)
                    weights_2 = copy.deepcopy(node["classifier"].fc2.weight.data)
                    bias_2 = copy.deepcopy(node["classifier"].fc2.bias.data)
                    weights_3 = copy.deepcopy(node["classifier"].fc3.weight.data)
                    bias_3 = copy.deepcopy(node["classifier"].fc3.bias.data)
                    classifier = define_solver(self.args.solver_ver, weights_1.shape[1], self.args.input_width, self.args.input_height, self.args.no_classes).cuda()
                    classifier.fc1.weight.data = weights_1
                    classifier.fc1.bias.data = bias_1
                    classifier.fc2.weight.data = weights_2
                    classifier.fc2.bias.data = bias_2
                    classifier.fc3.weight.data[:weights_3.shape[0],:] = weights_3
                    classifier.fc3.bias.data[:bias_3.shape[0]] = bias_3
                elif self.args.solver_ver == 6:
                    weights_1 = copy.deepcopy(node["classifier"].fc1.weight.data)
                    bias_1 = copy.deepcopy(node["classifier"].fc1.bias.data)
                    classifier = define_solver(self.args.solver_ver, weights_1.shape[1], self.args.input_width, self.args.input_height, self.args.no_classes).cuda()
                    classifier.fc1.weight.data[:weights_1.shape[0],:] = weights_1
                    classifier.fc1.bias.data[:bias_1.shape[0]] = bias_1
                del node["classifier"]
    ...
    def forward_for_da(self, input):
        da_out = self.tree_modules[0].transform(input)
        return da_out

    def forward_without_0(self, inputs):
        ...
        t_list = [inputs]  # transformed inputs
        ...
        while len(tmp_list):
            ...
            if self.tree_struct[idx]['split']:
                tmp_list.append(self.tree_struct[idx]['left_child'])
                tmp_list.append(self.tree_struct[idx]['right_child'])
            elif self.tree_struct[idx]['extended']:
                tmp_list.append(self.tree_struct[idx]['left_child'])
        # print(nodes_list)

        for i in range(len(nodes_list)):
            node = self.tree_struct[nodes_list[i]]
        # =============================
        # for node in self.tree_struct:
            ...
            if self.split and node['index'] == self.node_split:
                # print("if self.split and node['index'] == self.node_split:", node['index'])
                ...
                r_list_final.append(ro * p_left)
                r_list_final.append(ro * (1.0 - p_left))
                

            elif self.extend and node['index'] == self.node_extend:
                # print("elif self.extend and node['index'] == self.node_extend:", node['index'])
        ...
        y_pred = 0.0
        for r, s in zip(r_list_final, s_list):
            y_pred += r * torch.exp(s)

        out = torch.log(1e-10 + y_pred)

        # print(r_list[0].sum()/128)
        # print(r_list[1].sum()/128)
        # input()
        if self.training:
...
class MLP_LeNet(nn.Module):
    ...
    """
    def __init__(self, input_nc, input_width, input_height, no_classes, **kwargs):
        ...
        self.fc1 = nn.Linear(input_nc, 120)
        self.fc2 = nn.Linear(120, 84)
        self.fc3 = nn.Linear(84, no_classes)

    def forward(self, x):
        # out = x.view(x.size(0), -1)
        out = x.mean(dim=-1).mean(dim=-1).squeeze()  
...
class MLP_LeNetMNIST(nn.Module):
    ...
    """
    def __init__(self, input_nc, input_width, input_height, no_classes=10, dropout_prob=0.0, **kwargs):
...
class Solver_GAP_TwoFClayers(nn.Module):
    ...
    def __init__(self, input_nc, input_width, input_height, no_classes=10, 
        ...
        self.fc1 = nn.Linear(input_nc, input_nc/reduction_rate + 1)
        self.fc2 = nn.Linear(input_nc/reduction_rate + 1, no_classes)
        self.sigmoid = nn.Sigmoid()
...
#         self.fc2 = nn.Linear(32, no_classes)
