...
def define_solver(version, input_nc, input_width, input_height, no_classes, **kwargs):
    if version == 1:  # Logistric regressor
        return models.LR(input_nc, input_width, input_height, no_classes, **kwargs)
    elif version == 2:  # MLP with 2 hidden layers:
        return models.MLP_LeNet(input_nc, input_width, input_height, no_classes, **kwargs)
    elif version == 3:  # MLP with a single hidden layer (MNIST LeNet)
        return models.MLP_LeNetMNIST(input_nc, input_width, input_height, no_classes, **kwargs)
    elif version == 4:  # GAP + 2 FC layers
        return models.Solver_GAP_TwoFClayers(input_nc, input_width, input_height, no_classes, **kwargs)
    elif version == 5:  # MLP with a single hidden layer in AlexNet
        return models.MLP_AlexNet(input_nc, input_width, input_height, no_classes, **kwargs)
    elif version == 6:  # GAP + 1 FC layer
        return models.Solver_GAP_OneFClayers(input_nc, input_width, input_height, no_classes, **kwargs)
    else:
...
def get_scheduler(scheduler_type, optimizer, grow):
    if scheduler_type == 'step_lr': # reduce the learning rate
        ...
        #     optimizer, milestones=[15, 30, 45], gamma=0.2,
        # )
        scheduler = optim.lr_scheduler.MultiStepLR(
            optimizer, milestones=[25,45], gamma=0.2,
        )
        ...
        if grow: # use 'plateau' during the local growth phase
            scheduler = optim.lr_scheduler.ReduceLROnPlateau(
                optimizer, 'min', factor=0.2, patience=10,
            )
        else:
            scheduler = optim.lr_scheduler.MultiStepLR(
                optimizer, milestones=[15, 30, 45], gamma=0.2,
